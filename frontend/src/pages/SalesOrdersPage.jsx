...
import { Edit2, Trash2, Search, Calendar, TrendingUp } from 'lucide-react';

const SalesOrdersPage = ({ salesOrders = [], products = [], suppliers = [], onAdd, onUpdate, onDelete, addToast }) => {
  const [formData, setFormData] = useState({ product_id: '', supplier_id: '', qty: '', price: '' });
  ...
  const startEdit = (so) => {
    ...
    const prod = products.find(p => p.product_id === so.product_id);
    setProductSearch(prod ? `${prod.product_id} - ${prod.name} (Stock: ${prod.qty})` : `Prod ID: ${so.product_id}`);
    const supp = suppliers.find(s => s.supplier_id === so.supplier_id);
    setSupplierSearch(supp ? `${supp.supplier_id} - ${supp.name}` : `Entity ID: ${so.supplier_id}`);
  };

  const cancelEdit = () => {
    setEditId(null);
    setFormData({ product_id: '', supplier_id: '', qty: '', price: '' });
    setProductSearch('');
    setSupplierSearch('');
  };
  ...
  const handleProductSelection = (product) => {
    setFormData({
      ...
      // Auto-populate price with standard 20% markup
      price: (Number(product.price) * 1.20).toFixed(2)
    });
    setProductSearch(`${product.product_id} - ${product.name} (Stock: ${product.qty})`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.product_id || !formData.supplier_id || formData.qty === '' || formData.price === '') {
      addToast('All fields are required, including valid Product and Entity selections', 'error');
      return;
    }

    const qtySold = Number(formData.qty);
    const selectedProd = products.find(p => p.product_id === formData.product_id);

    // Perform local stock validation
    if (!editId && selectedProd && Number(selectedProd.qty) < qtySold) {
      addToast(`Error: Insufficient stock. Only ${selectedProd.qty} units available.`, 'error');
      return;
    }

    if (editId) {
      const success = await onUpdate('sales-orders', editId, {
      ...
      const success = await onAdd('sales-orders', {
        product_id: formData.product_id,
        supplier_id: formData.supplier_id,
        qty: qtySold,
        price: Number(formData.price)
  ...
  const handleDelete = (id) => {
    if (window.confirm(`Are you sure you want to delete Sales Order ID "${id}"?`)) {
      onDelete('sales-orders', id);
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const ids = filteredOrders.map(so => so.so_id);
      setSelectedIds(ids);
    } else {
      setSelectedIds([]);
    }
  ...
  const handleBatchDelete = async () => {
    if (window.confirm(`Are you sure you want to delete the ${selectedIds.length} selected sales orders?`)) {
      let successCount = 0;
      let failCount = 0;
      for (const id of selectedIds) {
        const success = await onDelete('sales-orders', id);
        if (success) successCount++;
        else failCount++;
      }
      setSelectedIds([]);
      if (failCount > 0) {
        addToast(`Deleted ${successCount} sales orders. ${failCount} orders could not be deleted.`, 'error');
      } else {
        addToast(`Successfully deleted ${successCount} sales orders.`, 'success');
      }
    }
  };

  const filteredOrders = salesOrders.filter(so => 
    so.so_id.toString().includes(searchQuery) ||
    so.product_id.toLowerCase().includes(searchQuery.toLowerCase()) ||
    so.supplier_id.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className="p-6 rounded-2xl border border-slate-200/80 dark:border-white/5 bg-white/70 dark:bg-slate-900/30 backdrop-blur-md shadow-sm dark:shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5 relative">
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Product</label>
            <input
              type="text"
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.slice(0, 10).map((p) => (
                    ...
                    >
                      ...
                      <span>{p.name}</span>
                      <span className="block text-[0.675rem] text-slate-400 dark:text-slate-505 mt-0.5">Stock: {p.qty} | Price: ₹{(Number(p.price) * 1.2).toFixed(2)}</span>
                    </div>
                  ))
                )}
              </div>
            ...
            {selectedProduct && (
              <span className={`text-[0.675rem] font-bold mt-1 ${Number(selectedProduct.qty) <= 10 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                Available: {selectedProduct.qty} units
              </span>
            )}
          </div>

          <div className="flex flex-col gap-1.5 relative">
            <label className="text-xs font-semibold text-slate-555 dark:text-slate-400">Customer / Entity</label>
            <input
              type="text"
            ...
            {showSupplierDropdown && (
              <div className="absolute top-[102%] left-0 right-0 max-h-48 overflow-y-auto z-20 mt-1 bg-white dark:bg-slate-955 border border-slate-200 dark:border-white/10 rounded-xl shadow-lg dark:shadow-2xl backdrop-blur-lg">
                {filteredSuppliers.length === 0 ? (
                  <div className="p-3 text-xs text-slate-400 dark:text-slate-505 italic">No suppliers/entities found</div>
                ) : (
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-555 dark:text-slate-400">Quantity Sold</label>
            <input
              type="number"
              className="w-full px-4 py-2.5 text-sm rounded-xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-505 focus:outline-none focus:border-indigo-500/50 dark:focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all"
              placeholder="e.g. 5"
              ...
              min="1"
              max={selectedProduct ? selectedProduct.qty : undefined}
              required
            />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-555 dark:text-slate-400">Sale Price Per Unit (₹)</label>
            <input
              type="number"
              step="0.01"
              className="w-full px-4 py-2.5 text-sm rounded-xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-505 focus:outline-none focus:border-indigo-500/50 dark:focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 transition-all"
            ...
            {formData.qty && formData.price && (
              <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold mt-1">
                Revenue: ₹{(Number(formData.qty) * Number(formData.price)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </span>
            )}
          </div>
      ...
      <div className="p-6 rounded-2xl border border-slate-200/80 dark:border-white/5 bg-white/70 dark:bg-slate-900/30 backdrop-blur-md shadow-sm dark:shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-white/5 pb-4 mb-5">
          <div className="flex items-center gap-3">
            <span className="text-sm font-bold text-slate-800 dark:text-slate-100">View Sales Orders ({filteredOrders.length})</span>
          ...
          <div className="relative w-full sm:max-w-[240px]">
            ...
            <input
              type="text"
              placeholder="Search by SO ID, Product, Customer..."
        ...
        <div>
          {filteredOrders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300">No Sales Orders</h3>
              <p className="text-xs text-slate-400 dark:text-slate-555 mt-1 max-w-[260px]">No sales orders match your search query or database is empty.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="sticky top-0 z-10 bg-slate-100/90 dark:bg-slate-950/95 backdrop-blur-md">
                    ...
                    <th className="pb-3 pt-1.5 font-semibold text-slate-550 dark:text-slate-400 border-b border-slate-200 dark:border-white/5 uppercase tracking-wider">SO ID</th>
                    ...
                    <th className="pb-3 pt-1.5 font-semibold text-slate-550 dark:text-slate-400 border-b border-slate-200 dark:border-white/5 uppercase tracking-wider">Customer / Client</th>
                    ...
                    <th className="pb-3 pt-1.5 font-semibold text-slate-550 dark:text-slate-400 border-b border-slate-200 dark:border-white/5 uppercase tracking-wider">Total Revenue</th>
                    <th className="pb-3 pt-1.5 font-semibold text-slate-550 dark:text-slate-400 border-b border-slate-200 dark:border-white/5 uppercase tracking-wider">Sale Date</th>
                ...
                <tbody>
                  {filteredOrders.map((so) => {
                    const product = products.find(p => p.product_id === so.product_id);
                    const supplier = suppliers.find(s => s.supplier_id === so.supplier_id);
                    return (
                      <tr key={so.so_id} className="hover:bg-slate-100/50 dark:hover:bg-white/[0.01] transition-all">
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5 text-center">
                          <input 
                            type="checkbox" 
                            className="rounded border-slate-300 dark:border-white/10 bg-white dark:bg-slate-900 text-indigo-600 focus:ring-indigo-500 cursor-pointer w-3.5 h-3.5"
                            checked={selectedIds.includes(so.so_id)}
                            onChange={() => handleSelectRow(so.so_id)}
                          />
                        </td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5 text-slate-800 dark:text-slate-200 font-bold">#{so.so_id}</td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5">
                          <div className="flex flex-col">
                            <span className="text-slate-800 dark:text-slate-200 font-medium">{product ? product.name : so.product_id}</span>
                            <span className="text-[0.7rem] text-slate-400 dark:text-slate-505 mt-0.5">ID: {so.product_id}</span>
                          </div>
                        </td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5">
                          <div className="flex flex-col">
                            <span className="text-slate-700 dark:text-slate-300">{supplier ? supplier.name : so.supplier_id}</span>
                            <span className="text-[0.7rem] text-slate-400 dark:text-slate-505 mt-0.5">ID: {so.supplier_id}</span>
                          </div>
                        </td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5">
                          <span className="inline-block px-2.5 py-0.5 rounded-full font-semibold text-[0.675rem] bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400">
                            {so.qty} units
                          </span>
                        </td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5 text-slate-700 dark:text-slate-300 font-mono">₹{Number(so.price).toFixed(2)}</td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5 text-emerald-600 dark:text-emerald-400 font-bold font-mono">
                          ₹{(Number(so.qty) * Number(so.price)).toFixed(2)}
                        </td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5 text-slate-550 dark:text-slate-400 font-medium">
                          <div className="flex items-center gap-1.5">
                            <Calendar size={12} className="text-slate-400 dark:text-slate-550 shrink-0" />
                            <span>{new Date(so.sale_date).toLocaleDateString()}</span>
                          </div>
                        </td>
                        <td className="py-3.5 border-b border-slate-200 dark:border-white/5">
